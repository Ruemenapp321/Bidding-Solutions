...
function persistAnalysis(body) { const next = { ...lightweightAnalysis(storedAnalysis()), ...lightweightAnalysis(body), progress_percent: Math.max(lastConfirmedProgress, Number(body.progress_percent ?? body.progress ?? 0)) }; localStorage.setItem(analysisStateKey, JSON.stringify(next)); }
