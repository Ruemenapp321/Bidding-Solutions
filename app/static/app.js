...
function recovery(item) { if (!item.redeye || item.redeye === 'none') return ['No WOCL departure', 'neutral']; const max = Math.max(...(item.duty_legs || []), 0); if (max <= 1) return ['WOCL · Easy recovery', 'excellent']; if (max === 2) return ['WOCL · Moderate recovery', 'strong']; if (max === 3) return ['WOCL · Heavy recovery', 'fair']; return ['WOCL · Demanding recovery', 'low']; }
...
function render() {
  ...
  shown.forEach((item, index) => {
    const [label, cls] = rating(item), [rec, recCls] = recovery(item), layovers = (item.layovers || []).map(x => `${x.city}${x.duration ? ` ${x.duration}` : ''}`).join(', ') || 'No overnights', reasons = (item.reasons || []).slice(0, 6), conflicts = item.calendar_conflicts || [], pay = payPresentation(item);
  ...
  if (allResults.length) { const top = allResults[0], topPay = payPresentation(top), [topRating] = rating(top), [topRecovery] = recovery(top); $('summary').textContent = `${term.analyzed}: ${allResults.length} · showing ${shown.length}. Ratings reflect your selected preferences.`; $('snapshotMatch').textContent = topRating; $('snapshotPayLabel').textContent = topPay.snapshotLabel; $('snapshotCredit').textContent = topPay.snapshotValue || '—'; $('snapshotLength').textContent = top.duty_legs && top.duty_legs.length ? `${top.duty_legs.length}-day` : '—'; $('snapshotRecovery').textContent = topRecovery.replace(' recovery', ''); }
}
